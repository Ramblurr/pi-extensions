 ...
 *
 * The generated prompt opens in $VISUAL or $EDITOR for review/editing,
 * then appears as a draft in the new pi session editor.
 */
...
async function editWithExternalEditor(
	...
	return ctx.ui.custom<string | undefined>((tui, theme, _kb, done) => {
		const openingMessage = new Text(
			`${theme.fg("accent", title)}\n${theme.fg("muted", `Opening external editor: ${editorCmd}`)}`,
			1,
			1,
		);
		...
		setImmediate(() => {
			...
			try {
				tmpDir = mkdtempSync(join(tmpdir(), "pi-handoff-"));
				const tmpFile = join(tmpDir, "handoff.md");

				writeFileSync(tmpFile, prefill, "utf8");
				tui.stop();
				stopped = true;

				const result = spawnSync(`${editorCmd} ${quoteShellArg(tmpFile)}`, {
					shell: true,
					stdio: "inherit",
				});

				if (result.status === 0) {
					edited = readFileSync(tmpFile, "utf8").replace(/\n$/, "");
				} else {
					const reason = result.error
						...
						: result.signal
						  ...
						  : result.status !== null
						    ? ` (exit ${result.status})`
						    : "";
					console.error(`External editor exited without saving${reason}`);
				}
			} catch (err) {
...
export default function (pi: ExtensionAPI) {
	pi.registerCommand("handoff", {
		...
		handler: async (args, ctx) => {
			...
			// Let user edit the generated prompt in their external editor.
			const editedPrompt = await editWithExternalEditor(ctx, "Edit handoff prompt", result);
